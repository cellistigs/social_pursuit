'''
WARNING: LEGACY CODE, NO TESTS WRITTEN 11/11/20 
Raw import of module from Video Pipelining into social-pursuit repo. This will probably cause errors, 
'''
...
from social_pursuit.labeled import LabeledData as training_dataset
...
class Social_Dataset(object):
    def __init__(self,filepath,vers = 0,networkname = None):
        self.dataset = pd.read_hdf(filepath)
        self.dataset_name = filepath.split('/')[-1]
        if networkname is None:
            self.scorer = 'DeepCut' + os.path.basename(filepath).split('DeepCut')[-1].split('.h5')[0]
        self.scorer = networkname 
    ...
    def render_trajectory_nan(self,pindex):
        """Returns a trajectory set with nans in the rejected locations instead of points.  
        ...
        coords = np.concatenate((x,y),axis = 1)
        out = rawtrajectories[time,coords]
        nans = np.empty_like(rawtrajectories[:,:2])
        nans[:] = np.nan
        local_coords = coords%3
        nans[time,local_coords] = rawtrajectories[time,coords]
        return nans
    ...
    def render_trajectories_nan(self,to_render = None):
        if to_render == None:
            to_render = self.part_index
        part_trajectories = []
        for pindex in to_render:
            part_traj = self.render_trajectory_nan(pindex)
            part_trajectories.append(part_traj)
        animalstack = np.stack([np.stack(part_trajectories[:5],axis = -1),np.stack(part_trajectories[5:],axis = -1)],axis = -1)
...
## 4/25/19: Try to replace all instaces of filter_full with filter_full_new!
    def filter_full_new(self,trainpath):
        """Filtering function written 4/25/19 that first segments traces into plausible pieces, and then stitches plausible pieces back together. 
