...
class FourierDescriptor():
    ...
    def get_contour_image(self,animal):
        """INTESTED Given an animal, gets a nan image filled in only by that animal.

        :param animal: string identifying the animal that we will be using 
        """
        interp = self.normalized_interpolation(self.contours[animal].shape[0]*5)[animal].T
        rotated = interp

        ## Finally, get the image underneath the contour:
        integer_contour = np.round(rotated).astype(int)
        ## Round down to the image shape:
        reference_shape = np.array([self.image[:,:,0].shape]).T-1
        rounded_contour = np.minimum(integer_contour,reference_shape)
        ## Zero mask:
        zeros_mask = np.zeros_like(self.image[:,:,0])
        zeros_mask[rounded_contour[0,:],rounded_contour[1,:]] = 1
        zeros_mask = ndi.binary_fill_holes(zeros_mask)
        if self.image.dtype == np.dtype('uint8'):
            image = self.image/255.
        else:
            image = self.image

        
        only_mouse = np.zeros_like(image)
        only_mouse[:] = np.nan
        only_mouse[zeros_mask,:] = image[zeros_mask,:]


        return only_mouse

    def superimpose_centered_points(self,animal,points):
        """The analogue to superimpose_centered_contour for body part points. 
        ...
        angle = (self.mouseangles[identitydict[animal]]-np.pi/2) ## rotate towards the vertical axis.
        rotmatrix = np.array([[np.cos(angle),np.sin(angle)],[-np.sin(angle),np.cos(angle)]])
        rotated = np.matmul(rotmatrix,points)
        ## Recenter:
        recentered = rotated+self.points[:,3:4,identitydict[animal]]
        return recentered
...
class PoseDistribution():
    ...
    def stack_pose(self,vectorized_markers):
        ...
        """
        reshaped = vectorized_markers.reshape(2,4,order = "F")
        withcent = np.insert(reshaped,3,axis = 1,values = 0)
        return withcent
