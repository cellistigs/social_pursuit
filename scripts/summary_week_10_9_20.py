...
if __name__ == "__main__":
    ...
    todolist = [
            ...
            [
                "x Render all existing data in 2-level polar representation "+md.new_inline_link(link = "./polar_representation.md",text = "(here)"),
                ...
                "Build in mouse-specific data into representation with marked training points",
                    ["x Extract out training points (`social_pursuit.data.labeled.LabeledData.dataarray`)",
                     "x Extract out image contours from training frames "+md.new_inline_link(link = "./get_statistical_shape_model.md",text = "(here)"),
                     "x Calculate fourier descriptors on the image (`social_pursuit.data.labeled.LabeledData.get_contour_fourier_rep`) and turn into statistical shape model (mean and variance in fourier space) "+md.new_inline_link(link = "./from_contours_to_shape_model.md",text = "(here)"),
            ...
            "Denoising Revisited",
            [
                "x Write algorithm to classfiy trajectories as clean/problematic based on 1) existence of switches, and 2) entire loss of other animal with clustering algorithms on relative bodypart angle."+md.new_inline_link(link = "./polar_classification.md",text = " (see here)"),
                "Segment with clustering algorithms on the angles of body parts",
                    ["x Build mechanism to generate noisy surrogate data for testing "+md.new_inline_link(link = "./polar_classification_validation.md",text = "(here)"),
    ...
    md.new_paragraph("*Why should we care so much about image contours? Why invest so much energy in a preprocessing step for denoising?")
    md.new_line("I think that using fourier descriptors for animal contours can be much more than preprocessing for denoising, and it is in fact a natural extension of the work we are already doing that open the door to a lot of new analyses if we take it as a more central idea than markers. As I see it: ")
    md.new_list(["Our analysis thus far is a special case of fourier descriptor analysis, where we have just looked at the DC component to localize the centroid of the animal.",
